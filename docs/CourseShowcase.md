...
这是一个手势识别方向的计算机视觉课程实践。当前 Python/CV 主线已经包含数据集读取、模型训练、评估、单图推理、摄像头实时手势展示、性能测试和展示资产生成。

核心分类模型已经从 LeNet 基线升级为改进 CNN，输入为 `28x28` 灰度手势图像，输出为 Sign Language MNIST 的 24 个静态字母类别。`J` 和 `Z` 需要动态轨迹，因此不属于当前静态分类任务。现场摄像头展示使用 MediaPipe 手部关键点做稳定实时手势识别，并通过 Python/OpenCV 看板把手势映射到真实鼠标/键盘控制；CNN 字母分类保留为训练评估成果。
...
- 有实时应用入口：`infer_camera.py` 使用 MediaPipe 先检测手部，展示模式可实时识别 `Open Palm`、`Point`、`Pinch`、`Victory`、`Fist`，并可安全开启真实鼠标/键盘控制；字母模式可裁剪 ROI 后做 24 类分类。
...
5. 最后运行 `./run_showcase_demo.sh`，先说明安全锁定，再按 `c` 开启真实控制，依次展示移动、点击/拖拽、切换窗口和重置。
